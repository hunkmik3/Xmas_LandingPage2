    ...
    if (overlayImg) {
        ...
        async function checkGiftsAndUpdate() {
            ...
            try {
                ...
                // Cập nhật top-bar: thời gian hiện tại + số quà còn lại
                const topBar = document.getElementById('top-bar-text');
                if (topBar) {
                    const nowTop = new Date();
                    const pad = (n) => (n < 10 ? '0' + n : '' + n);
                    const timeStr = `${pad(nowTop.getHours())}:${pad(nowTop.getMinutes())} ${pad(nowTop.getDate())}/${pad(nowTop.getMonth() + 1)}/${nowTop.getFullYear()}`;
                    topBar.textContent = `${timeStr} - Quà còn lại: ${isNaN(giftsLeft) ? '?' : giftsLeft}`;
                }
